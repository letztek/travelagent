...
## Phase 1: AI 核心邏輯 (Route Planner) [checkpoint: b0c83ca]
...
## Phase 2: 前端預覽 UI
- [x] Task: 建立路線預覽組件
    - [x] 實作 `RouteFlow` 元件（時間軸或流程圖樣式）。
    - [x] 展示規劃理由 (Rationale)。
- [x] Task: 建立路線預覽頁面
    - [x] `/requirements/[id]/route` 頁面。

## Phase 3: 流程整合 (The Big Loop)
- [x] Task: 串接 Gap Wizard 至 Route Preview
    - [x] 當對話補全完成後，自動導向至路線預覽頁。
- [x] Task: 修改詳細行程生成按鈕
    - [x] 將原本列表頁的「生成行程」改為先進入「預覽路線」。
    - [x] 在路線預覽頁提供「確認並生成詳細行程」按鈕。

## Phase 4: 驗收與優化
- [x] Task: 測試多目的地情境的路線合理性
- [x] Task: 優化 UI 互動（例如：點擊節點顯示簡單摘要）
